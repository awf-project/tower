...
| [user-guide/debug-sessions.md](user-guide/debug-sessions.md) | How to configure and drive interactive Debug Adapter Protocol sessions, one-shot `tower_debug_eval_at` probes, and rr-backed reverse-debug workflows |
...
| [user-guide/semantic-edits.md](user-guide/semantic-edits.md) | How to preview and apply LSP rename and AST anchored symbol edits through Tower's host-owned apply-edits path |
...
| [mcp-tools.md](mcp-tools.md) | Wire protocol, session lifecycle, the native `tower_*` tools, extension tools (`tower_ast_get_outline`, `tower_ast_replace_symbol_body`, `tower_lsp_rename`, `tower_lint_check`, `tower_debug_launch`, ...), JSON-RPC error codes |

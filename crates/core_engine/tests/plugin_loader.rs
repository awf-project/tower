...
use core_engine::adapters::{InMemoryFs, PluginLoadError, WasmtimeHost};
...
fn ac1_load_hello_returns_plugin_instance() {
    ...
    assert_eq!(manifest.tools.len(), 2, "AC1: two tools");
...
fn ac1_loaded_instance_registers_in_domain_registry() {
    ...
    assert_eq!(
        tools.len(),
        2,
        "AC1: registry must contain greet and read_file_echo tools"
    );
...
fn ac3_unknown_host_import_is_link_error() {
    let result = WasmtimeHost::load(forbidden_host_import_wat(), empty_fs());
...
fn u2_wasi_fd_write_resolves_but_is_inert_in_sandbox() {
    ...
    let load_result = WasmtimeHost::load(forbidden_import_wasm(), empty_fs());
...
fn ac4_abi_mismatch_plugin_is_rejected() {
    let result = WasmtimeHost::load(abi_mismatch_wasm(), empty_fs()).map(|_| "<plugin instance>");
